...
class SecurityScanner:
    ...
    def check_firewall(self):
        ...
        Previously checked `'active' in output.lower()` - "inactive"
        contains "active" as a substring, so a fully disabled firewall
        (`Status: inactive`) was being reported as ACTIVE. Verified this
        with real ufw output before fixing it. Now checks for the exact
        phrase "status: active" instead, which "status: inactive" does
        not contain.
        """
        try:
            result = subprocess.run(['ufw', 'status'], capture_output=True, text=True)
            status = 'status: active' in result.stdout.lower()
    ...
    def check_ssh_security(self):
        ...
        Previously used substring checks like `'PermitRootLogin' in
        content and 'no' in content` - this does not verify the actual
        VALUE of any directive. Since the word "no" appears almost
        anywhere in a real sshd_config (comments, other directives,
        ordinary words), and 'PasswordAuthentication' in content just
        checks the directive NAME is present regardless of whether it's
        set to yes or no, this could (and did, verified) report a
        completely insecure config - PermitRootLogin yes,
        PasswordAuthentication yes - as "SECURE". This now actually
        parses each directive's value.
        """
        try:
            ...
            for line in content.split('\n'):
                ...
                if len(parts) == 2:
                    directives[parts[0].lower()] = parts[1].strip().lower()

            # PubkeyAuthentication defaults to "yes" if unset (OpenSSH
            # default), so an absent directive should not count against
            # the check the way an absent PermitRootLogin/
            # PasswordAuthentication (which default to more permissive
            # values) should.
            checks = {
                'PermitRootLogin no': directives.get('permitrootlogin', '').startswith('no'),
                'PasswordAuthentication no': directives.get('passwordauthentication', '').startswith('no'),
                'PubkeyAuthentication yes': directives.get('pubkeyauthentication', 'yes').startswith('yes'),
            }
